...
## 設計思想：「完成画像が主役、要確認だけ修正」

Media Factoryは「入力ツール」ではなく「AIによる自動生成ツール」です。この方針は今後追加する**すべて**のテンプレート（試合結果速報・最終順位・大会案内など）に共通で適用します。

- 確認画面で最初に見せるのは編集フォームではなく、テンプレートの**完成プレビュー画像**（`PreviewGallery.tsx` の仕組み）
- 全項目のフォームを並べて手入力させない。AIが読み取れなかった項目だけを `要確認：N件` のようなコンパクトなチェックリストとして抽出し、その場でピンポイント修正できるようにする（`lib/reviewItems.ts` の `buildReviewItems` / `ReviewChecklist.tsx`）
- チェックリストの各項目は「対象を一意に特定できるラベル＋値」の形（例：`No.B10 チームA`）。埋めた瞬間にリストから自動的に消える（`onBlur` で確定し、件数が減っていく）
- 開催日・会場・チームの追加削除など、チェックリストでは表現できない**構造変更**のためのフォールバックとして、元のフルフォームは折りたたみ済みの「詳細編集」に残す（普段は開かない）

新しいテンプレートを追加する際は、`lib/templateSections.ts` の `buildExportUnits`／`buildSections` にエントリを増やし、`lib/reviewItems.ts` にそのテンプレート用の要確認項目抽出ロジックを追加する形にしてください。ウィザードの構成自体（アップロード→解析→確認→PNG出力）は変更不要です。
