...
- **対戦カード（チーム名）の自動入力には、タイムテーブルだけでなく「組み合わせ表」「抽選結果」「トーナメント表」などチーム名が載った資料を一緒にアップロードしてください。** タイムテーブル単体には試合No.と時間しか載っていないことが多く、AIはそこにないチーム名を作り出すことはできません。試合No.（例："B15"）を手がかりに資料同士を横断照合し、対戦カードを埋めます
- 組み合わせ表が「勝ち上がり方式のトーナメント表」（1回戦・2回戦…と段階的に対戦相手が決まる形式）の場合も、1回戦などすでに実名が印字されている試合はその実名を、まだ対戦相手が決まっていない試合は資料の表記（例：「B1の勝者」）をそのまま対戦カードとして自動入力します。「まだ決まっていない」という事実そのものが読み取り結果であり、要確認扱いにはしません
- トーナメント表テンプレート（`bracket`、8チーム単一トーナメント専用）の自動読み取りは、出場8チームの単一トーナメント（プール分けなし・不戦勝なし）の場合のみ対応します。それ以外の複雑な形式は`bracket`としては空のままですが、上記の対戦カード自動入力（`matches[].teamA/teamB`）には影響しません
- 複雑な組み合わせ表の読み取りには多段階の推論が必要なため、解析には`thinking`（拡張思考）とストリーミング応答を使用しています。1回のAPI呼び出しが数十秒〜数分かかることがあります
- 解析APIは `server/extractHandler.ts` の `runExtraction` に共通化されており、ローカル開発では `server/viteExtractPlugin.ts`（Vite開発サーバー用）、本番デプロイでは `api/extract.ts`（Vercel Serverless Function）がそれぞれ同じ `/api/extract` パスで呼び出します。Vercel側の関数実行時間は `vercel.json` で `maxDuration: 300`（5分）に設定しています（Hobbyプランでは短く制限される場合があるため、複雑な資料を扱う場合はProプラン以上を推奨します）
